  ...
  return (
    <article className="group rounded-3xl border border-white/5 bg-slate/80 p-5 transition hover:-translate-y-1 hover:border-white/20">
      <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-ink/50">
        ...
        <div className="absolute left-4 top-4 text-[0.6rem] uppercase tracking-[0.3em] text-white/60">
          {product.category}
        </div>
      </div>
      <div className="mt-4 space-y-2">
        ...
        <div className="flex gap-3 pt-4">
          <button
            type="button"
            className="flex-1 rounded-full border border-white/10 bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-white transition hover:border-white/30"
            onClick={() =>
              addItem({
